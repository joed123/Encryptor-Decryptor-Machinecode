This program (Encryptor.asm) accepts three parameters which it then passes onto the stack, the requirments chnage depdning on the mode its in. 
If its in decoy mode it accepts, a 16 bit signed WORD operand, a 16 bit signed WORD operand and a 32 bit OFFSET of a signed DWORD (the sum will be placed into the specified DWORD).
